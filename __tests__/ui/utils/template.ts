import {template} from '../../../src/ui/utils/template';
...
describe('template', () => {
	...
	test('vars', () => {
		expect(
			template('{animal} eat {thing}', {animal: 'Black bear', thing: 'Battlestar Galactica'})
	...
	test('edge case', () => {
		...
		expect(
			template('thing{thing}', {thing: 'Battlestar Galactica'})
		).toEqual(['Battlestar Galactica', 'Battlestar Galactica']);
	});
});
